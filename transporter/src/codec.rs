use crate::{HEAD_LENGTH, err::ErrorCode};
use tokio_util::{
...
impl Decoder for TransportCodec {
    ...
    fn decode(&mut self, buf: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        let head = &buf[..HEAD_LENGTH];
        let version = head[0];
        let index: u16 = head[1..3].try_into().map(u16::from_be_bytes).unwrap();
        let body_length: u32 = head[3..HEAD_LENGTH]
            .try_into()
        ...
        if (buf.len() as u32) < (HEAD_LENGTH as u32 + body_length) {
            // 包不够长（遇到了典型的 TCP 半包）
            // 顺手优化：预先告诉底层池子，我还差多少，让池子一次性扩容，别挤牙膏
            buf.reserve(HEAD_LENGTH + (body_length as usize) - buf.len());
            return Ok(None);
        }

        // 已凑齐一帧数据，进行零拷贝切割
        let mut full_frame = buf.split_to(HEAD_LENGTH + (body_length as usize));

        // 头部已解析出来，直接丢弃
        full_frame.advance(HEAD_LENGTH);
