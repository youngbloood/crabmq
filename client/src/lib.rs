...
struct PartitionAssignment {
    // 分区 ID -> PartitionInfo
    pub partitions: IndexMap<u32, commonsvc::PartitionInfo>,
...
impl PartitionAssignment {
    // 返回下一个 partitio -> broker_addr
    fn next(&self) -> Option<&commonsvc::PartitionInfo> {
    ...
    fn by_key(&self, k: &str) -> Option<&commonsvc::PartitionInfo> {
        if k.is_empty() {
            return self.next();
        }
    ...
    fn hash_key(&self, key: &str) -> Option<&commonsvc::PartitionInfo> {
        let key_binding =
...
impl TopicAssignment {
    ...
    fn apply_topics(&self, tp: commonsvc::TopicPartition) {
        for (k, v) in &tp.assignments {
            self.assignments
                .entry(k.to_string())
                .and_modify(|pa| {
                    for pi in &v.partitions {
                        pa.partitions.insert(pi.id, pi.clone());
                    }
                })
                .or_insert_with(|| -> PartitionAssignment {
                    let mut pa = PartitionAssignment::default();
                    pa.max_partition = Arc::new(AtomicUsize::new(v.partitions.len() as _));
                    for pi in &v.partitions {
                        pa.partitions.insert(pi.id, pi.clone());
                    }
                    pa
...
impl Client {
    ...
    fn apply_topic_list(&self, tp: commonsvc::TopicPartition) {
        self.topics.apply_topics(tp);
    }
    ...
    fn get_partition(
        ...
        key: &str,
        auto_map: bool,
    ) -> Result<Option<commonsvc::PartitionInfo>> {
...
impl Publisher<'_> {
    ...
    {
        let partition = self.client.get_partition(&self.topic, "", auto_create)?;
        ...
        let endpoint = self.get_or_create_channel(&partition.addrs[0]).await?;
            ...
            loop {
                ...
                select! {
                    bts = rx.recv() => {
                        ...
                        let partition_id = client.get_partition(&_topic, &key, auto_create).unwrap().unwrap().id;
                        let _ = tx_middleware.send( PublishReq{
                            topic: _topic.clone(),
