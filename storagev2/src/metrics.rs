...
impl StorageWriterMetrics {
    ...
    pub(crate) fn inc_flush_count(&self, data_count: u64, index_count: u64) {
        if data_count != 0 {
            self.data_flush_count
                .fetch_add(data_count, Ordering::Relaxed);
        }
        if index_count != 0 {
            self.index_flush_count
                .fetch_add(data_count, Ordering::Relaxed);
        }
    }
    ...
    pub(crate) fn inc_flush_bytes(&self, data_num: u64, index_num: u64) {
        ...
        if index_num != 0 {
            self.index_flush_bytes
                .fetch_add(data_num, Ordering::Relaxed);
        }
    }
