    ...
    SR: StorageReader,
{
    ...
    async fn handle_broker_coo_communication(&mut self, coo_leader: Arc<Mutex<String>>) {
        ...
        // 初始化通信状态
        let mut current_communication_mode = CommunicationMode::External;
        ...
        loop {
            ...
            // 如果通信模式需要切换
            if current_communication_mode != target_mode {
                info!(
                    "Broker[{}] switching communication mode: {:?} -> {:?}",
                    broker_id, current_communication_mode, target_mode
                );
                ...
                // 切换到新的通信模式
                current_communication_mode = target_mode;
